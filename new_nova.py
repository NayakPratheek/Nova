...
#Function to search queries in wikipedia
...
def process_command(command):
    if "what is the time" in command or "what's the time" in command:
        ...
        speak("what do u want to search in wikipedia?")
        sear = get_command().lower()
        result = search_on_wikipedia(sear)
        page = wikipedia.page(sear)
        url = page.url
        print(f"Url:{url}")
        speak(f"According to wikipedia..,{result}")
        print(result)
        speak("To know more.. Click on the above provided url")

    elif "can you tell me" in command:
        # General-purpose query handling
        query = command.replace("can you tell me", "").strip()
        gemini_result = search_in_gemini(query)
        speak(f"According to Gemini AI: {gemini_result}")
